...
export async function GET(
    ...
    try {
        ...
        const response = await fetch(`${apiUrl}/api/job-seekers/${id}`, {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) {
            return NextResponse.json(
                { success: false, message: data.message || 'Failed to fetch job seeker' },
                { status: response.status }
            );
        }

        // For debugging, log the parsed data structure
        console.log("Successfully parsed job seeker data:", JSON.stringify(data).substring(0, 200) + "...");
...
export async function PUT(
    ...
    try {
        ...
        // Make a request to your backend API
        const apiUrl = process.env.API_BASE_URL || 'http://localhost:8080';
        console.log(`Making PUT request to: ${apiUrl}/api/job-seekers/${id}`);
        ...
        if (!response.ok) {
            return NextResponse.json(
                {
                    success: false,
                    message: data.message || 'Failed to update job seeker',
                    error: data
                },
                { status: response.status }
            );
...
export async function DELETE(
    ...
    try {
        const { id } = await params;
        console.log(`DELETE request received for job seeker ID: ${id}`);
        ...
        // Make a request to your backend API
        const apiUrl = process.env.API_BASE_URL || 'http://localhost:8080';
        console.log(`Making DELETE request to backend at: ${apiUrl}/api/job-seekers/${id}`);
